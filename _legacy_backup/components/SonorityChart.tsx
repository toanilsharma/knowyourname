...
import { SonorityPoint } from '../types';
...
export const SonorityChart: React.FC<Props> = ({ data }) => {
  ...
  const getX = (index: number) => {
    return (index / (data.length - 1)) * (100 - (padding * 2 / 5)) + (padding / 5); // Approximate percentage logic
  };

  // Y coordinate calculation (invert score because SVG Y=0 is top)
  const getY = (score: number) => {
    // Score 1 (Low) -> Height - padding
    // Score 10 (High) -> padding
