...
Analyzed via KnowYourName.co.in`;
  ...
  const handleDownloadImage = async (format: 'story' | 'square' | 'landscape') => {
    ...
    try {
      ...
      // NATIVE SHARE (Mobile)
      if (format === 'story' && navigator.canShare && navigator.canShare({ files: [new File([], 'test.png')] })) {
        const blob = await (await fetch(image)).blob();
        const file = new File([blob], `KYN-Story-${data.name}.png`, { type: 'image/png' });

        try {
          await navigator.share({
            files: [file],
            title: 'My Name Analysis',
            text: `Check out my linguistic aura: knowyourname.co.in/?name=${data.name}`
          });
          setGenerating(null);
      ...
      link.href = image;
      const suffix = format === 'square' ? 'Instagram' : format === 'landscape' ? 'LinkedIn' : 'Story';
