...
import { Activity, Music, Mic2, PlayCircle } from 'lucide-react';
import { Interpolator } from '@/lib/interpretations';
import { speakName } from '@/lib/audio';
    ...
    >
      <div className="grid lg:grid-cols-3 gap-8 items-center">
        ...
        <div className="lg:col-span-2 relative h-[200px] w-full bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700/50 p-4">
          <div className="absolute top-2 left-4 flex items-center gap-1.5 z-20">
            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Sonority Curve</span>
            <div className="group/tooltip relative flex items-center justify-center w-3.5 h-3.5 rounded-full bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400 text-[8px] cursor-help font-bold">
              ?
              <div className="absolute top-full left-0 mt-2 w-56 p-2 bg-slate-900 text-white text-[10px] leading-relaxed rounded-lg shadow-xl opacity-0 invisible group-hover/tooltip:opacity-100 group-hover/tooltip:visible transition-all z-50 pointer-events-none text-left font-normal normal-case tracking-normal">
                <div className="absolute -top-1 left-2 w-2 h-2 bg-slate-900 rotate-45"></div>
                Curve calculated using the <strong>Sonority Sequencing Principle (Clements, 1990)</strong>, mapping acoustic resonance from vowels (highest) to voiceless stops (lowest).
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-between mt-[-10px] px-0 relative z-10 w-full group/graph">
            {data.map((p, i) => (
              <button
                key={i}
                onClick={() => speakName(p.char)}
                className="flex flex-col items-center relative transition-transform hover:-translate-y-1 hover:scale-110 focus:outline-none"
                style={{ width: `${100 / data.length}%` }}
                title={`Click to hear isolated phoneme: ${p.char}`}
              >
                <span className="absolute -top-6 opacity-0 group-hover/graph:opacity-100 transition-opacity text-blue-500 drop-shadow-md">
